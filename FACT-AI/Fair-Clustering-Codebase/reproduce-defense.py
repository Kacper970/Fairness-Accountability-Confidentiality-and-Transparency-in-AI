...
import shutil
# import os
# os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:2' #':16:8' # set to ':4096:2' on cluster, ':16:8' locally

shutil.copy('Consensus-Fair-Clustering/models.py', './models.py')
shutil.copy('Consensus-Fair-Clustering/utils.py', './utils.py')

import warnings 
warnings.filterwarnings('ignore')

from fair_clustering.eval.functions import *
from fair_clustering.dataset import ExtendedYaleB, Office31, MNISTUSPS, FairFace
...
parser = argparse.ArgumentParser(description='Run defense experiments')
parser.add_argument('--dataset', choices=['MNIST_USPS', 'Office-31', 'Yale', 'DIGITS', 'FairFace'], type=str, required=True, help='Dataset to run defense experiments on')
parser.add_argument('--objective', choices=['balance', 'entropy'], type=str, required=True, default='balance', help='Objective to run defense experiments on')
parser.add_argument('--percent', type=int)
args = parser.parse_args()

objective = args.objective
name = args.dataset
run_percent = args.percent


print(f"Dataset: {name}, Objective: {objective}")
...
percentages = list(enumerate([int(0.125*len(U_idx_full)), int(0.25*len(U_idx_full)), int(0.375*len(U_idx_full)), int(0.5*len(U_idx_full)), int(0.625*len(U_idx_full)), int(0.75*len(U_idx_full)), int(0.875*len(U_idx_full)), int(len(U_idx_full))]))
if run_percent is not None:
   percentages = [percentages[run_percent]]

for percent, j in percentages:    
    print(f"{percent}/7")
    U_idx = U_idx_full[:j]
    V_idx = V_idx_full
...
if run_percent is None:
   run_percent = 'all'


with open('../reproduced-results/defense/' + name + '_' + str(run_percent) + '_' + objective + '.json', 'a') as f:
